...
export default function EmployeePatients() {
  ...
  const getPatientStats = (patientId: string) => {
    ...
    const pBills = bills.filter(b => b.status === 'unpaid' && (b.patientId === patientId || b.patientName === patients.find(p => p.id === patientId)?.name));
    
    const unpaidSum = pBills.reduce((sum, b) => sum + b.total, 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-800">قائمة المرضى (الاستقبال)</h2>
          <p className="text-slate-400 font-bold text-sm mt-1">عرض ملفات المرضى، الحجوزات، والديون المستحقة</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredPatients.length > 0 ? filteredPatients.map((p) => {
          ...
          return (
            <div
              key={p.id}
              onClick={() => router.push(`/dashboard/employee/patients/${p.id}`)}
